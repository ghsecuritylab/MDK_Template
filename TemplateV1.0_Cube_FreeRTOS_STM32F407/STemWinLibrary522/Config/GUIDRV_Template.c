...
#include "stm32f4xx.h"
#include "ra8870_app.h"

#define emWin_Speed_Up 1
...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
    ...
    GUI_USE_PARA(PixelIndex);
    {
      ...
      // TBD by customer...
      //
        lcd_write_point(xPhys, yPhys, PixelIndex);
    }
...
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  unsigned int PixelIndex;
    ...
    GUI_USE_PARA(y);
    {
      ...
      // TBD by customer...
      //
      PixelIndex = lcd_read_point(xPhys, yPhys);
    }
    #if (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
      #undef xPhys
      #undef yPhys
...
#if (emWin_Speed_Up == 1)
      lcd_bte_square_fill(x0, y0, y1 - y0 + 1, x1 - x0 + 1, PixelIndex);

//      temp = ((PixelIndex & 0xE000) >> 8) | ((PixelIndex & 0x0700) >> 6) | (PixelIndex & 0x0003);
//      lcd_draw_square_fill(x0, y0, x1, y1, temp);

//      lcd_memory_square_fill(x0, y0, x1, y1, PixelIndex);
#else
    for (; y0 <= y1; y0++) {
      for (x = x0; x <= x1; x++) {
...
#if (emWin_Speed_Up == 1)
    lcd_draw_hline(x0, y, x1, LCD_COLORINDEX);
#else
...
#if (emWin_Speed_Up == 1)
    lcd_draw_vline(x, y0, y1, LCD_COLORINDEX);
#else
...
#if (emWin_Speed_Up == 1)
    lcd_draw_hcolorline(x, y, x + xsize - 1, (uint16_t *)p);
#else
  for (;xsize > 0; xsize--, x++, p++) {
    _SetPixelIndex(pDevice, x, y, *p);
  }
