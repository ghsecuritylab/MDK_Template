...
/** 私有宏(类型定义) -------------------------------------------- */ 
#define NUM_LEDS (sizeof(led_gpio_config) / sizeof(led_gpio_config[0]))
    
/** 私有变量 --------------------------------------------------- */
/** 配置led用到的端口、引脚 */
const gpio_init_t led_gpio_config[] =
{
...
void led_on(uint16_t val) 
{
    ...
    for(n = 0; n < NUM_LEDS; n++)
    {
        if (val & (1 << n)) 
        {
            HAL_GPIO_WritePin(led_gpio_config[n].Port, led_gpio_config[n].Init.Pin, GPIO_PIN_SET);        
        }        
    }    
}
...
void led_off(uint16_t val) 
{
    ...
    for(n = 0; n < NUM_LEDS; n++)
    {
        if (val & (1 << n)) 
        {
            HAL_GPIO_WritePin(led_gpio_config[n].Port, led_gpio_config[n].Init.Pin, GPIO_PIN_RESET);        
        }        
    }     
}
